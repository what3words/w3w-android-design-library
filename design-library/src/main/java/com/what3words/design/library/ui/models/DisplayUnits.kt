 ...
 *
 * @param distanceKm The distance in kilometres. Non-finite values produce an empty string.
...
fun formatDistance(
    ...
    val converted = if (metric) distanceKm else distanceKm / KILOMETERS_PER_MILE
 ...
 *
 * Only 0 is ambiguous in whole kilometres, so only 0 renders `<1 km`.
 *
...
fun formatUnits(distanceKm: Int, displayUnits: DisplayUnits, context: Context): String {
    if (distanceKm != 0) return formatDistance(distanceKm.toDouble(), displayUnits)

    val unit =
        if (isMetricDisplayUnitEnabled(displayUnits)) MeasureUnit.KILOMETER else MeasureUnit.MILE
    val fmt = MeasureFormat.getInstance(Locale.getDefault(), MeasureFormat.FormatWidth.SHORT)
    return context.getString(R.string.distance_metric_low, fmt.format(Measure(1, unit)))
}
 ...
 */
private fun isMetricDisplayUnitEnabled(
